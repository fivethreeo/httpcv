...
::  Revision: $Id: build-icu_55.1.bat 4782 2015-08-23 20:06:01Z stathis $
::
:: Changelog:
::
:: - ICU ticket 11798 contains the patch necessary for building ICU 55.1 with MSVC2015
::   http://bugs.icu-project.org/trac/ticket/11798
::
:: - ICU ticket 10909 is still necessary for building ICU 55.1 with MSVC2008 (debug)
::   http://bugs.icu-project.org/trac/ticket/10909
::
...
SET PROJECT_VERSION=55.1

SET PROJECT_SRC_DIR=D:\opensource\icu-svn-!PROJECT_VERSION!

SET PROJECT_INSTALL_DIR=D:\dist\icu-dist-!PROJECT_VERSION!-vs!VS_VERSION!

:: %~dp0 represents the directory of this batch script. Meaning the .dat file must be in the same dir. 
:: This .dat file is optional, in case you want to build ICU with custom data tables.
:: If omitted the default will be used. For more info, see http://userguide.icu-project.org/icudata
:: If you want an ICU with only specific data tables included then use the online ICU Data Library 
:: Customizer to generate the custom .dat file at http://apps.icu-project.org/datacustom/
SET ICU_CUSTOM_DATA=%~dp0icudt55l.dat
...
IF "!SEVENZIP_CMD_OPTIONS!" == "" (
	SET SEVENZIP_CMD_OPTIONS=a -r -mx9 -mmt
)
...
pushd "!PROJECT_SRC_DIR!"
	...
	if /i "%~1" == "55.1" (
		call :applyPatch icu11798.patch
	)
...
pushd !DST_DIST_DIR!\..
	...
	echo Compressing with: !SEVENZIP_CMD! !SEVENZIP_CMD_OPTIONS! !COMPRESSED_FILE! !DST_DIST!
...
pushd !PROJECT_BUILD_DIR!
	...
	pushd !BUILD_DIR_DEST!
		...
		rem SET B_CMD=!B_CMD! --disable-layout
